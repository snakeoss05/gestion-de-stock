...
import { AlertTriangle, Package, TrendingDown, TrendingUp, Eye, History, RefreshCw } from "lucide-react"
...
export default function StockManager() {
  ...
  const { stockData, movements, lastRefresh, addStockMovement, refreshData, getStockAlerts } = useStock()
  ...
  const totalValue = stockData.reduce((sum, item) => sum + (item.value || 0), 0)
  const { lowStock: lowStockItems, outOfStock: outOfStockItems } = getStockAlerts()
      ...
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Dernière mise à jour: {lastRefresh.toLocaleTimeString("fr-FR")}</CardDescription>
            </div>
        ...
        <CardContent>
          ...
          <div className="flex gap-2 mb-6 flex-wrap">
            ...
            <Button variant={currentView === "alerts" ? "default" : "outline"} onClick={() => setCurrentView("alerts")}>
              <AlertTriangle className="h-4 w-4 mr-2" />
              Alertes
              {lowStockItems.length + outOfStockItems.length > 0 && (
                <Badge className="ml-2 bg-red-500">{lowStockItems.length + outOfStockItems.length}</Badge>
              )}
            </Button>
            <Button
          ...
          {currentView === "overview" && <StockOverview stockData={stockData} onStockUpdate={addStockMovement} />}
          {currentView === "movements" && <StockMovements movements={movements} stockData={stockData} />}
          {currentView === "alerts" && (
            <StockAlerts
              stockData={stockData}
          ...
          {currentView === "adjustment" && <StockAdjustment stockData={stockData} onStockUpdate={addStockMovement} />}
        </CardContent>
      </Card>
    </div>
  )
