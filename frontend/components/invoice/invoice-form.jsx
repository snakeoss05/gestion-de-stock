...
export default function InvoiceForm({
  ...
  const { getAvailableProducts } = useStock();
  const products = getAvailableProducts();

  const [formData, setFormData] = useState({
    type: "vente", // "vente" or "achat" or "retour"
    customerName: "",
    customerEmail: "",
    customerAddress: "",
  ...
  const handleScanResult = (result) => {
    if (result.type === "product") {
      ...
      const price =
        formData.type === "vente" ? product.prix_detail : product.prix_gros;

      const invoiceItem = {
        id: Date.now(),
        description: `${product.brandName} ${product.productName}`,
        quantity: 1,
        price: price,
        total: price,
      ...
      const newItems = [...formData.items, invoiceItem];
      setFormData((prev) => ({ ...prev, items: newItems }));
      calculateTotals(newItems);
    } else if (result.type === "barcode") {
      // Rechercher le produit par code-barres
      const product = products.find((p) => p.code_barre === result.data);
      if (product) {
        const price =
          formData.type === "vente" ? product.prix_detail : product.prix_gros;
    ...
    return (
      <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
        <UniversalBarcodeScanner
          ...
          description="Scannez un code-barres ou recherchez un produit à ajouter à la facture"
          mode="both"
          products={products}
      ...
      <Card className="shadow-lg border-0 bg-white/80 backdrop-blur-sm">
        <CardHeader className="bg-gradient-to-r from-green-600 to-teal-600 text-white rounded-t-lg">
          ...
          <CardDescription className="text-green-100">
            Définir les dates et le statut de la facture
          </CardDescription>
        </CardHeader>
        <CardContent className="p-6 space-y-4">
            ...
            <div
              ...
              <div className="space-y-2">
                ...
                <div className="flex items-center gap-2">
                  <Input
                    value={`${item.total.toFixed(2)} TND `}
                    readOnly
        ...
        <CardContent className="p-6 space-y-4">
          ...
          <div className="border-t pt-4 space-y-2">
            <div className="flex justify-between text-lg">
              ...
              <span>{formData.subtotal.toFixed(2)} TND </span>
            </div>
            <div className="flex justify-between text-lg">
              <span>TVA (20%):</span>
              <span>{formData.tax.toFixed(2)} TND </span>
            </div>
            <div className="flex justify-between text-xl font-bold border-t pt-2">
              <span>Total TTC:</span>
              <span>{formData.total.toFixed(2)} TND </span>
            </div>
          </div>
        </CardContent>
      </Card>
