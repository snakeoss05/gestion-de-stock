...
export default function StockOverview({ stockData, onStockUpdate }) {
  ...
  // Catégories uniques
  const categories = ["Tous", ...new Set(stockData.map((item) => item.category))]
  ...
  const getStockStatus = (item) => {
    if (item.currentStock === 0) {
      ...
      return { status: "Stock Élevé", color: "bg-blue-100 text-blue-800 border-blue-200" }
    } else {
                ...
                <tr key={item.id} className="border-b hover:bg-gray-50 transition-colors">
                  <td className="p-4">
                    <div>
                      ...
                      <div className="text-xs text-gray-400 font-mono">{item.code_barre}</div>
                    </div>
                  </td>
                  <td className="p-4">
                  ...
                  <td className="p-4">
                    ...
                    <div className="text-xs text-gray-500">Coût unitaire: {item.cost.toFixed(2)} €</div>
                  </td>
                  <td className="p-4">
